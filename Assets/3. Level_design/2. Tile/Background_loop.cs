...
public class Background_loop : MonoBehaviour
{
    ...
    {
        float objcetWidth = obj.GetComponent<SpriteRenderer>().bounds.size.x - choke;
        int childNeeded = (int)Mathf.Ceil(screenBounds.x * 2 / objcetWidth);
        ...
        for (int i = 0; i <= childNeeded; i++)
        {
            ...
            c.transform.position = new Vector3(objcetWidth * i + 1, obj.transform.position.y, obj.transform.position.z);
            c.name = obj.name + i;
        }
        Destroy(clone);
