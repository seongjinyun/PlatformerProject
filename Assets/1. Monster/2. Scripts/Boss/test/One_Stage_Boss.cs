...
public class One_Stage_Boss : Basic_Boss
{
    ...
    IEnumerator EarthGrow()
    {
        ...
        GameObject Skill_1_pos = Instantiate(Pre_EarthGrow, Target.position, Quaternion.Euler(0, 0, 0)); // 플레이어 위치에 준비 스킬뜨고
        yield return new WaitForSeconds(1f); // 1초뒤에
        Destroy(Skill_1_pos); // 준비 스킬 삭제
        GameObject Skill_1 = Instantiate(EarthGrowSkill, Target.position, Quaternion.Euler(0, 0, 0)); // 플레이어 위치에 스킬 뜸
